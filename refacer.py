...
# Ganti __import__('gc') dengan import biasa
import gc

# Preload NVIDIA DLLs if Windows - AMAN
...
# Daftar hash model yang dikenal aman (contoh - ganti dengan hash sebenarnya)
KNOWN_MODEL_HASHES = {
    'inswapper_128.onnx': 'sha256:1234567890abcdef...',  # Ganti dengan hash sebenarnya
    'det_10g.onnx': 'sha256:abcdef1234567890...',  # Ganti dengan hash sebenarnya
    'w600k_r50.onnx': 'sha256:0987654321fedcba...',  # Ganti dengan hash sebenarnya
}
...
def verify_file_integrity(filepath, expected_hash=None):
    ...
    if expected_hash is None:
        ...
        expected_hash = KNOWN_MODEL_HASHES.get(filename)
        if expected_hash is None:
            return True  # Skip jika tidak ada hash reference
    ...
    return sha256_hash.hexdigest() == expected_hash.split(':')[1] if ':' in expected_hash else sha256_hash.hexdigest() == expected_hash
...
class Refacer:
    ...
    def _partial_face_blend(self, original_frame, swapped_frame, face):
        """Blend partial face dengan aman"""
        try:
            ...
            print(f"⚠️ Error in _partial_face_blend: {e}")
            return swapped_frame

    def __download_with_progress(self, url, output_path):
        """Download file dengan verifikasi keamanan"""
        # Whitelist domain yang diperbolehkan
        allowed_domains = [
            'huggingface.co',
            'github.com',
            'raw.githubusercontent.com',
            'drive.google.com'  # Hati-hati dengan Google Drive
        ]
        ...
        if not any(domain == allowed or domain.endswith('.' + allowed) for allowed in allowed_domains):
            raise Exception(f"Domain {domain} tidak diizinkan untuk download")
        ...
        # Cek ukuran file wajar (maks 2GB)
        if total_size > 2 * 1024 * 1024 * 1024:
            raise Exception("File terlalu besar (>2GB)")
        ...
        # TODO: Tambahkan verifikasi hash jika ada

    def __check_providers(self):
        """Cek provider dengan aman"""
        ...
        if not os.path.exists(test_model):
            print("[INFO] Model belum di-download, akan didownload saat pertama kali digunakan")
    ...
    def __init_apps(self):
        """Inisialisasi aplikasi dengan aman"""
        try:
            assets_dir = ensure_available('models', 'buffalo_l', root='~/.insightface')

            model_path = os.path.join(assets_dir, 'det_10g.onnx')
            if not os.path.exists(model_path):
                raise Exception(f"Model {model_path} tidak ditemukan")
    ...
    def prepare_faces(self, faces, disable_similarity=False, multiple_faces_mode=False):
        ...
        for face in faces:
            ...
            # Validasi path file
            dest_path = face['destination']
            if not os.path.exists(dest_path):
                raise Exception(f'File tidak ditemukan: {dest_path}')

            _faces = self.__get_faces(dest_path, max_num=1)
            if len(_faces) < 1:
            ...
            if multiple_faces_mode:
                ...
                if "origin" in face and face["origin"] is not None and not disable_similarity:
                    origin_path = face['origin']
                    if not os.path.exists(origin_path):
                        raise Exception(f'File tidak ditemukan: {origin_path}')
                        
                    face_threshold = face['threshold']
                    bboxes1, kpss1 = self.face_detector.autodetect(origin_path, max_num=1)
                    if len(kpss1) < 1:
                        raise Exception('No face detected on "Face to replace" image')
                    feat_original = self.rec_app.get(origin_path, kpss1[0])
                else:
                    face_threshold = 0
    ...
    def __get_faces(self, frame, max_num=0):
        """Get faces dengan error handling"""
        try:
            ...
            print(f"⚠️ Error in __get_faces: {e}")
            return []

    def process_first_face(self, frame):
        """Process first face dengan aman"""
        try:
            ...
            print(f"⚠️ Error in process_first_face: {e}")
            return frame

    def process_faces(self, frame):
        """Process faces dengan aman"""
        try:
            faces = self.__get_faces(frame, max_num=0)
            if not faces:
                return frame
            ...
            print(f"⚠️ Error in process_faces: {e}")
            return frame

    def reface_group(self, faces, frames, output):
        """Reface group dengan ThreadPoolExecutor"""
        try:
            ...
            print(f"⚠️ Error in reface_group: {e}")
    ...
    def reface(self, video_path, faces, preview=False, disable_similarity=False, multiple_faces_mode=False, partial_reface_ratio=0.0):
        """Main reface function dengan aman"""
        try:
    ...
    def reface_image(self, image_path, faces, disable_similarity=False, multiple_faces_mode=False, partial_reface_ratio=0.0):
        ...
        try:
            ...
            print(f"❌ Error in reface_image: {e}")
            return None

    def extract_faces_from_image(self, image_path, max_faces=5):
        """Extract faces dengan aman"""
        try:
    ...
    def __try_ffmpeg_encoder(self, vcodec):
        """Try FFmpeg encoder dengan aman"""
        try:
            command = ['ffmpeg', '-y', '-f', 'lavfi', '-i', 'testsrc=duration=1:size=1280x720:rate=30', '-vcodec', vcodec, 'testsrc.mp4']
            subprocess.run(command, check=True, capture_output=True, timeout=30).stderr
            return True
        except (subprocess.CalledProcessError, subprocess.TimeoutExpired):
            return False
        finally:
            # Bersihkan file test
            if os.path.exists('testsrc.mp4'):
                try:
    ...
    def __check_encoders(self):
        ...
        try:
            ...
            commandout = subprocess.run(command, check=True, capture_output=True, timeout=30).stdout
            ...
            print(f"⚠️ Error checking encoders: {e}")
